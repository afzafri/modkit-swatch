...
| `code` | string | Yes | Product code (e.g. "C5", "XF-8") |
| `name` | string | Yes | Paint color name |
| `hex` | string | Yes | Hex color, 6-digit lowercase with `#` |
| `finish` | string | Yes | gloss, flat, semi-gloss, metallic, or clear |
| `type` | string | Yes | lacquer, acrylic, enamel, or water-based acrylic |
| `suitableFor` | object | Yes | Application method suitability |
| `suitableFor.airbrush` | boolean | Yes | Suitable for airbrush |
| `suitableFor.handPainting` | boolean | Yes | Suitable for hand painting |
...
The app attempts to load the image and falls back to a hex color swatch if the
file does not exist.
